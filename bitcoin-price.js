...
function setTitle() {
    ...
    var price = BitcoinPrice.getPrice(curr);
    chrome.browserAction.setTitle({
        'title': '1 Bitcoin = ' + price + " " + curr
    });
};
...
function setBadge(multiplicator) {
    ...
    var price = BitcoinPrice.getPrice(curName);
    price = price / d;
...
function setPriceInHistory(newPrice) {
    ...
    if (store.get('lastMax') === null) {
        ...
        if ((options.notificationMax.get() === true) && newPrice > options.lastMax.get()) {
            notify('New maximum Bitcoin price', 'The highest price is now ' + newPrice);
        }
    ...
    if (store.get('lastMin') === null) {
        ...
        if ((options.notificationMin.get() === true) && newPrice < options.lastMin.get()) {
            notify('New minimum Bitcoin price', 'The lowest price is now ' + newPrice);
        }
...
function launchInterval() {
    ...
    period = period * 1000;
    bitcoinInt = setInterval(function () {
...
var BitcoinPrice = {
    ...
    getPrice: function (currency) {
        var value = JSON.parse(store.get('currentBitcoinRate'))[currency];
        return value;
    },
            ...
            return AJAX
                ...
                .then(function (data) {
                    if (data) {
                        ...
                        if(Object.keys(BitcoinPrice.currencyExchangesRates).length==0){
                            BitcoinPrice.currencyExchangesRates= JSON.parse(store.get('currencyExchangesRates'));
                        }
                        for (var cur in BitcoinPrice.currencyExchangesRates) {
                            BitcoinPrice.currentBitcoinRate[cur] = parseFloat(BitcoinPrice.currencyExchangesRates[cur]) * (btcInUSD);
                        }
                        store.set('currentBitcoinRate', JSON.stringify(BitcoinPrice.currentBitcoinRate));
                        return true;
                    }else{
                        return false;
                    }
            ...
            return AJAX
                ...
                .then(function (data) {
                    ...
                    if (xml.getElementsByTagName('Cube')[2].attributes['currency'].value == "USD") {
                        ...
                        BitcoinPrice.currencyExchangesRates['USD'] = 1;
                        BitcoinPrice.currencyExchangesRates['EUR'] = valueEURUSD;
                        for (var i = 3; i < xmlList.length; i++) {
                            ...
                            BitcoinPrice.currencyExchangesRates[cur] = rat * valueEURUSD;
                        }
                        store.set('currencyExchangesRates', JSON.stringify(BitcoinPrice.currencyExchangesRates));
                        
                        return BitcoinPrice.currencyExchangesRates;
                    }
                });
        }
    }
...
var store = {
    validateIntegrity: function () {
        ...
        var valid = true;
        var keysList = ['currency', 'currencyExchangesRates', 'divider', 'history', 'lastMax', 'lastMin', 'notificationMax', 'notificationMin', 'precision', 'refresh', "monitorWealth",'wealth'];
